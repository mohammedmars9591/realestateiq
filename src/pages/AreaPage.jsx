...
const AreaPage = () => {
  ...
  const rentalAnalysisPoints = [
    ...
    `Proximity to schools like ${conn.school?.name || "local academies"} and leisure facilities enhances overall rental appeal.`
  ];

  const salesAnalysisPoints = [
    `Off-plan properties in ${area.name} are gaining popularity due to flexible payment plans and potential for capital appreciation.`,
    "The community's mature infrastructure and amenities drive strong demand for resale properties.",
    "Limited new supply in the area supports sustained price growth and value retention."
  ];
      ...
      <div ref={reportRef} className="bg-slate-50 p-1 md:p-4 rounded-3xl"> 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-16">
          ...
          <div className="lg:col-span-2 space-y-8">
              ...
              {/* REAL ESTATE IN AREA (Luxury Text Card) */}
              <div className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm">
                 <h3 className="text-2xl font-serif font-bold text-slate-900 mb-4 border-b border-slate-200 pb-4">
                   Real Estate in {area.name}
                 </h3>
                 <p className="text-slate-600 text-sm leading-8 text-justify font-serif">
                    {realEstateDesc}
                 </p>
              </div>

              {/* MARKET OVERVIEW TABLE (Financial Report Style) */}
              <div className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm">
              ...
              {/* RENTAL & SALES ANALYSIS GRID */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 <div className="bg-gradient-to-br from-blue-50 to-white border border-blue-100 rounded-3xl p-6 shadow-sm">
                    <h4 className="text-lg font-serif font-bold text-blue-900 mb-4 border-b border-blue-200 pb-2">Rental Analysis</h4>
                    <ul className="space-y-3">
                       {rentalAnalysisPoints.map((point, i) => (
                          <li key={i} className="flex items-start gap-3 text-sm text-slate-700 leading-relaxed">
                             <div className="mt-1.5 w-1.5 h-1.5 rounded-full bg-blue-500 flex-shrink-0"></div><span>{point}</span>
                          </li>
                       ))}
                    </ul>
                 </div>
                 <div className="bg-gradient-to-br from-emerald-50 to-white border border-emerald-100 rounded-3xl p-6 shadow-sm">
                    <h4 className="text-lg font-serif font-bold text-emerald-900 mb-4 border-b border-emerald-200 pb-2">Sales Analysis</h4>
                    <ul className="space-y-3">
                       {salesAnalysisPoints.map((point, i) => (
                          <li key={i} className="flex items-start gap-3 text-sm text-slate-700 leading-relaxed">
                             <div className="mt-1.5 w-1.5 h-1.5 rounded-full bg-emerald-500 flex-shrink-0"></div><span>{point}</span>
                          </li>
                       ))}
                    </ul>
                 </div>
              ...
              {/* RENTAL MARKET INTELLIGENCE (KPI Dashboard) */}
              <div className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm">
                 ...
                 <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="p-4 bg-blue-50 rounded-2xl border border-blue-100 text-center">
                       <div className="text-[10px] uppercase font-bold text-blue-600 mb-1">Avg. Gross Yield</div>
              ...
              {/* SUPPLY PIPELINE & UPGRADES */}
              <div className="bg-slate-900 text-white rounded-3xl p-8 shadow-xl relative overflow-hidden">
                 <div className="relative z-10">
                    <div className="flex justify-between items-start mb-8">
                       <div className="flex items-center gap-3">
                          <HardHat className="text-amber-400" />
                          <h3 className="text-xl font-serif font-bold">Supply & Master Plan</h3>
                       </div>
                    ...
                    <div className="mb-8">
                       <div className="flex justify-between text-sm font-medium text-slate-300 mb-2"><span>Completion Status</span><span>{supplyStats.handedOver} Complete</span></div>
                       <div className="w-full h-2 bg-slate-800 rounded-full overflow-hidden flex"><div className="h-full bg-emerald-500" style={{ width: '85%' }}></div><div className="h-full bg-amber-500" style={{ width: '15%' }}></div></div>
                    </div>

                    <div className="grid grid-cols-1 gap-4">
                       {futureInfra.map((upgrade, index) => (
                          <div key={index} className="flex items-center justify-between p-4 bg-white/10 rounded-xl border border-white/5 backdrop-blur-sm">
                             <div className="flex items-center gap-3"><div className="w-1.5 h-1.5 bg-amber-500 rounded-full animate-pulse"></div><span className="font-bold text-sm text-slate-100">{upgrade.name}</span></div>
                             <span className="text-xs font-mono text-amber-400 bg-amber-500/10 px-2 py-1 rounded">{upgrade.status}</span>
                          </div>
                       ))}
                    </div>
                 </div>
              ...
              <div className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm">
                 <h3 className="text-xl font-extrabold text-slate-900 mb-6 flex items-center gap-2"><BookOpen className="text-blue-600"/> Investment Thesis</h3>
                 <div className="space-y-6">
                    ...
                    <div className="w-full h-px bg-slate-100"></div>
                    <div><h4 className="flex items-center gap-2 text-sm font-bold text-slate-900 uppercase tracking-wide mb-2"><Lightbulb size={16} className="text-amber-500"/> Advisor Tips</h4><p className="text-slate-600 text-sm leading-relaxed text-justify">{narratives.uaeTips}</p></div>
                 </div>
              </div>

              {/* ATTRACTIONS (Luxury Cards) */}
              <div className="space-y-6">
                 <h3 className="text-2xl font-serif font-bold text-slate-900 border-b border-slate-200 pb-4">Lifestyle & Amenities</h3>
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm hover:shadow-md transition-shadow">
                       <div className="flex items-center gap-3 mb-6"><h4 className="text-xl font-serif font-bold text-slate-800">Waterfront & Leisure</h4></div>
                       <ul className="space-y-4">{leisureItems.slice(0,3).map((item, i) => (<li key={i} className="flex items-start gap-3"><div className="mt-1.5 w-1.5 h-1.5 rounded-full bg-teal-500"></div><span className="text-sm text-slate-600 font-medium">{item}</span></li>))}</ul>
                    </div>
                    <div className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm hover:shadow-md transition-shadow">
                       <div className="flex items-center gap-3 mb-6"><h4 className="text-xl font-serif font-bold text-slate-800">Shopping & Dining</h4></div>
                       <ul className="space-y-4">{shoppingItems.slice(0,3).map((item, i) => (<li key={i} className="flex items-start gap-3"><div className="mt-1.5 w-1.5 h-1.5 rounded-full bg-purple-500"></div><span className="text-sm text-slate-600 font-medium">{item}</span></li>))}</ul>
                    </div>
                 </div>
              </div>
              ...
              <div className="bg-slate-900 text-white rounded-3xl p-8 shadow-xl relative overflow-hidden">
                 <div className="relative z-10">
                    <div className="flex items-center gap-3 mb-8"><Navigation className="text-blue-400" /><h3 className="text-xl font-serif font-bold">Connectivity</h3></div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                       <div className="bg-white/10 p-5 rounded-2xl border border-white/10 backdrop-blur-sm"><div className="text-blue-300 mb-3"><Car size={28} /></div><div className="text-[10px] font-bold uppercase opacity-60 tracking-wider mb-1">ROAD NETWORK</div><div className="font-bold text-lg leading-tight mb-2">{roadAccess}</div><p className="text-xs text-slate-400">Seamless connectivity via major arterial highways.</p></div>
                       <div className="bg-white/10 p-5 rounded-2xl border border-white/10 backdrop-blur-sm"><div className="text-blue-300 mb-3"><Train size={28} /></div><div className="text-[10px] font-bold uppercase opacity-60 tracking-wider mb-1">PUBLIC TRANSIT</div><div className="font-bold text-lg leading-tight mb-2">{conn.metro?.name}</div><p className="text-xs text-slate-400">Well-integrated public transport network.</p></div>
                       <div className="bg-white/10 p-5 rounded-2xl border border-white/10 backdrop-blur-sm"><div className="text-blue-300 mb-3"><Bike size={28} /></div><div className="text-[10px] font-bold uppercase opacity-60 tracking-wider mb-1">WALKABILITY</div><div className="font-bold text-lg leading-tight mb-2">{walkability}</div><p className="text-xs text-slate-400">Designed for an active lifestyle.</p></div>
                    </div>
                 </div>
                 <div className="absolute inset-0 opacity-10 bg-[url('https://upload.wikimedia.org/wikipedia/commons/e/ec/World_map_blank_without_borders.svg')] bg-cover bg-center"></div>
              </div>
                 ...
                 <div className="overflow-x-auto -mx-8 px-8 pb-4">
                   <table className="w-full min-w-[700px] text-left border-collapse">
                     ...
                     <tbody className="text-sm font-medium text-slate-700">
                       <tr className="border-b border-slate-50"><td className="py-4 font-bold">Studio</td><td className="px-4">380 sq.ft</td><td className="px-4 text-xs">Open plan</td><td className="px-4"><span className="bg-blue-100 text-blue-700 px-2 py-1 rounded text-[10px] font-bold">YIELD</span></td><td className="px-4 font-bold text-emerald-600">{economics.studio?.roi}</td><td className="pl-4">{economics.studio?.rent}</td></tr>
                       <tr className="border-b border-slate-50"><td className="py-4 font-bold">1-Bed</td><td className="px-4">750 sq.ft</td><td className="px-4 text-xs">Balcony</td><td className="px-4"><span className="bg-purple-100 text-purple-700 px-2 py-1 rounded text-[10px] font-bold">LIQUIDITY</span></td><td className="px-4 font-bold text-emerald-600">{economics.oneBed?.roi}</td><td className="pl-4">{economics.oneBed?.rent}</td></tr>
                       <tr><td className="py-4 font-bold">2-Bed</td><td className="px-4">1100 sq.ft</td><td className="px-4 text-xs">En-suite</td><td className="px-4"><span className="bg-amber-100 text-amber-700 px-2 py-1 rounded text-[10px] font-bold">STABILITY</span></td><td className="px-4 font-bold text-emerald-600">{economics.twoBed?.roi}</td><td className="pl-4">{economics.twoBed?.rent}</td></tr>
                     </tbody>
                   </table>
                 </div>
              </div>
          ...
          <div className="space-y-8">
              ...
              {/* STRATEGIC CONNECTIVITY (List) */}
              <div className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm">
                 <div className="flex items-center gap-3 mb-6"><MapPin className="text-blue-600" /><h3 className="text-xl font-serif font-bold text-slate-900">Strategic Connectivity</h3></div>
                 <div className="space-y-6">
                   ...
                   <DistanceRow icon={<Train />} category="Metro / Transport" data={conn.metro} color="text-emerald-600" />
                   <DistanceRow icon={<GraduationCap />} category="Top School" data={conn.school} color="text-amber-500" />
